...
export function useBattleState() {
  ...
  function sync() {
    if (engine.value && engine.value.battleOver) return
  ...
  function _initEngine(enemiesInput) {
    ...
    _handlingDefeat = false
     destroyEngine() // 该函数内部应清空 engine.value
  // 原来可能还有这行，确保有
  ...
  async function enemyTurn(showMessage) {
    ...
    for (const enemy of alive) {
      ...
      if (enemy.isBoss && res.messages.length > 0) showMessage(res.messages[0], 1000)
  ...
  function victory() {
    ...
    _victoryCalled = true
    gameOver.value = true; gameOverMsg.value = '战斗胜利！'
    if (engine.value) { engine.value.battleOver = true; engine.value.winner = 'player' }
    handleVictory(engine.value)
    waiting.value = false; playerTurn.value = false; showSkillPanel.value = false
    if (!isRaidBattle.value) setTimeout(() => { showResult.value = true }, 100)
  }
  ...
  setTimeout(() => {
    ...
    showRaidResult.value = true
    _handlingDefeat = false // 解锁，允许后续重试
  }, 800)
} else {
  _handlingDefeat = false
}
        ...
        setTimeout(() => { _handlingDefeat = false }, 500)
    }
  }
