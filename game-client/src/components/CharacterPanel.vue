...
<div class="engrave-dots">
  ...
  ></span>
  <span
    v-for="i in Math.max(0, 10 - eff.level)"
    :key="'b' + i"
    class="dot"
  ></span>
...
// Boss 伤害（可通过刻印/套装效果汇总，这里从 playerStats 中取，若没有则从装备计算）
const bossDmg = computed(() => {
  // 直接取 store.playerStats.specialBossDmg 或 activeSetBonuses 的汇总
  return store.playerStats.specialBossDmg || 0
})
const bossDmgBonus = computed(() => {
  // 理论上刻印/装备加成会反映在 playerStats 中，这里简单返回 0，等待数据接入
  return 0
})
// 刻印图标映射（命运方舟风格）
