  ...
  <div class="overlay" @click.self="$emit('close')">
    <div class="panel">
      ...
      <div class="rank-header">
        ...
        <div class="rank-info">
          <h2>{{ rankName }}</h2>
          <div class="rank-stage">商店折扣 {{ discountPercent }}%</div>
        </div>
      </div>
      ...
      <div v-if="store.pendingRankUp" class="rank-up-banner">
        ...
        <span>升段讨伐已自动接受，击败指定Boss即可晋升！</span>
      </div>
...
const displayExp = computed(() => {
  ...
  return Math.min(exp, currentRankRequiredExp.value)
})
...
const availableMonsters = computed(() => {
  return monsterTemplates.value.filter(m => availableMonsterIds.value.has(m.id) && !m.isBoss && m.id !== 'training_dummy')
})
...
function getMonsterReward(monster) {
  ...
  const rankMult = 1 + (currentRank.value.requiredExp / 200) // 段位加成
  return {
    exp: Math.floor((60 + baseLv * 10) * mult * rankMult),
...
function getMaterialReward(mat) {
  ...
  const rankMult = 1 + (currentRank.value.requiredExp / 200)
  return {
    exp: Math.floor((40 + price / 4) * rankMult),
