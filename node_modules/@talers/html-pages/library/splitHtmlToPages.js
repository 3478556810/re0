import { breakingSpaceCharacters, getUnbreakableSlice, isElement, makeAllParentListElementsTransparent, yieldToMainThread } from "./utilities.js";
/**
 ...
 */ export async function* splitHtmlToPages(html, options = {}, signal) {
  ...
  const waitForDebuggingPurposes = (ms = options.debugWaitingTime ?? 1_000)=>{
    if (options.debug) {
  ...
  nextPage: while(source.firstChild){
      ...
      if (container.clientHeight > pageHeight) {
        ...
        if (sourceNode.nodeType === Node.TEXT_NODE) {
          ...
          if (offset === -1 || offset === sourceText.length) {
            ...
            targetNode.textContent = sourceText.slice(0, offset);
            const isBreakingSpace = breakingSpaceCharacters.includes(sourceText[offset]);
            sourceNode.textContent = offset === -1 ? sourceText : sourceText.slice(isBreakingSpace ? offset + 1 : offset);
          }
          makeAllParentListElementsTransparent(sourceNode);
          yield container.innerHTML;
          await toNextPage();
        ...
        if (isElement(sourceNode) && isElement(source) && source.tagName === "OL") {
    ...
    if (source !== root) {
      console.error("Should never happen: reached the end of the HTML but source is not root. This is most likely a bug.", {
        html
      });
    }
...
//# sourceMappingURL=splitHtmlToPages.js.map